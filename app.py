...
app = Flask(__name__)
app.config['SECRET_KEY'] = 'eg_optimal_secure_final_2026'
...
def verify_captcha(response):
    secret = "6Lct67gpAAAAADX-G2T_C_K8pS1oR-Y8M0qB7p9-"
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, request.form.get('password')):
            if not user.is_confirmed:
                flash("Lütfen hesabınızı mail üzerinden onaylayın.")
...
def register():
    if request.method == 'POST':
        if not verify_captcha(request.form.get('g-recaptcha-response')):
            flash("Lütfen güvenliği doğrulayın.")
        ...
        try:
            ...
            msg = Message("EG Optimal Aktivasyon", recipients=[email], body=f"Hesap onay linki: {url}")
            mail.send(msg)
            flash("Kayıt başarılı! Aktivasyon mailini kontrol edin.")
...
def confirm_email(token):
    try:
        ...
        db.session.commit()
        flash("Hesabınız onaylandı!")
    except: flash("Onay linki geçersiz.")
    return redirect(url_for('login'))
...
def forgot_password():
    if request.method == "POST":
        ...
        if user:
            ...
            msg = Message("EG Optimal Şifre Sıfırlama", recipients=[email], body=f"Şifre sıfırlama linki: {url}")
            try: mail.send(msg)
            except: pass
...
@login_required
def export_excel():
    df = pd.DataFrame([{'Kategori': e.category, 'Başlık': e.title} for e in Entry.query.filter_by(user_id=current_user.id).all()])
    output = BytesIO()
