...
app = Flask(__name__)
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', 'erhan-strateji-anahtari-2026')
...
# ============================================================
# VERİTABANI MODELLERİ (Hiçbir Sütunu Atlama)
# ============================================================
class User(UserMixin, db.Model):
    ...
    company_name = db.Column(db.String(100)) # Adminin düzeltebileceği alan
    is_admin = db.Column(db.Boolean, default=False)
    is_confirmed = db.Column(db.Boolean, default=False)
    created_at = db.Column(db.Date, default=date.today) # 30 gün kontrolü için
...
# ============================================================
# ÖDEME VE DENEME SÜRESİ KİLİDİ
# ============================================================
def access_denied():
    if current_user.is_authenticated and not current_user.is_admin:
        days_passed = (date.today() - current_user.created_at).days
        # 30 günü geçmiş ve ödeme yapılmamışsa engelle
        if days_passed > 30 and current_user.payment_status != 'Odendi':
            return True
    return False

# ============================================================
# ADMIN VE KURUM YÖNETİMİ
# ============================================================
...
def admin_panel():
    ...
    user_objects = {u.id: u for u in User.query.all()}
    return render_template('admin.html', users=users, entries=all_entries, bugun=bugun, user_objects=user_objects)
...
def update_payment(uid):
    if current_user.is_admin:
        ...
        u.company_name = request.form.get('company_name') # İSİM DÜZELTME BURADA
...
# ============================================================
# KULLANICI ROTALARI (SİL, EKLE, EXCEL)
# ============================================================
@app.route('/dashboard')
@login_required
def dashboard():
    if access_denied():
        flash('Deneme süreniz dolmuştur. Lütfen ödeme yapınız.', 'danger')
...
@login_required
def sertifikalar():
    if access_denied(): return redirect(url_for('login'))
...
def sil(id):
    ...
    if item and (item.user_id == current_user.id or current_user.is_admin):
        ...
        flash('Kayıt başarıyla silindi.', 'info')
...
def export_excel():
    ...
    data = [{"Firma": e.firma_adi, "Belge": e.title, "Bitiş Tarihi": e.expiry_date} for e in entries]
...
# ============================================================
# GÜVENLİK (MAİL ONAY, ŞİFRE SIFIRLAMA)
# ============================================================
...
def confirm_email(token):
    try:
        ...
        flash('Onay linki geçersiz.', 'danger'); return redirect(url_for('login'))
...
def reset_password(token):
    try:
        ...
        flash('Link geçersiz.', 'danger'); return redirect(url_for('forgot_password'))
    if request.method == 'POST':
        user = User.query.filter_by(email=email).first()
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password, request.form.get('password')):
            if not user.is_confirmed:
                flash('Lütfen önce mailinizi onaylayın.', 'warning'); return redirect(url_for('login'))
...
def kayit():
    if request.method == 'POST':
        ...
        if User.query.filter_by(email=email).first():
            flash('Bu mail zaten kayıtlı.', 'warning'); return redirect(url_for('kayit'))
        ...
        if not is_patron:
            ...
            msg.body = f"Üyeliğinizi tamamlamak için linke tıklayın: {confirm_url}"
            mail.send(msg); flash('Onay maili gönderildi. Lütfen kutunuzu kontrol edin.', 'info')
        return redirect(url_for('login'))
